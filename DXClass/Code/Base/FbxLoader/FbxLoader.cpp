...
std::unique_ptr<Model> FbxLoader::LoadModelFromFile(const string& modelName)
{
...
{
    // ノード名を取得
    string fbxName = fbxNode->GetName();
    ...
    node.scale = { (float)scale[0],(float)scale[1] ,(float)scale[2] ,0.0f };
    ...
    // スケール、回転、平行移動行列の計算
    XMMATRIX matScale, matRotation, matTranslation;
    matScale = XMMatrixScalingFromVector(node.scale);
    matRotation = XMMatrixRotationRollPitchYawFromVector(node.rotation);
    matTranslation = XMMatrixTranslationFromVector(node.translation);

    // ローカル変換行列の計算
    node.transform = XMMatrixIdentity();
    node.transform *= matScale; // ワールド行列にスケールを反映
        ...
        if (material)
        {
            ...
            if (diffuseProperty.IsValid())
            {
                ...
                if (texture)
                {
                    ...
                    // テクスチャ読み込み
                    LoadTexture(model, baseDirectory + model->fbxName + "/" + name);
                    textureLoaded = true;
                }
            }
        }
...
void FbxLoader::ParseSkin(Model* model, FbxMesh* fbxMesh)
{
    ...
    for (int i = 0;i < clusterCount;i++)
    {
        ...
        // XMMatrix型に変換する
        XMMATRIX initPose;
        ConvertMatrixFromFbx(&initPose, fbxMat);
        // 初期姿勢行列の行列を得る
        bone.invInitPose = XMMatrixInverse(nullptr, initPose);
    }
