...
void PostEffect::Draw(ID3D12GraphicsCommandList* cmdList)
{
	/*if (input->TriggerKey(DIK_0))
	{
		...
		srvDesc.ViewDimension = D3D12_SRV_DIMENSION_TEXTURE2D; // 2Dテクスチャ
		srvDesc.Texture2D.MipLevels = 1;
		dev->CreateShaderResourceView(texBuff[tex].Get(), // ビューと関連図けるバッファ
			&srvDesc,
...
void PostEffect::BeforeDrawScene(ID3D12GraphicsCommandList* cmdList)
{
	...
	for (int i = 0;i < 2;i++)
	{
		viewports[i] = CD3DX12_VIEWPORT(0.0f, 0.0f, WindowApp::winWidth, WindowApp::winHeight);
		scissorRects[i] = CD3DX12_RECT(0, 0, WindowApp::winWidth, WindowApp::winHeight);
	}
	// ビューポートの設定
